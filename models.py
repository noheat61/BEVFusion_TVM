...
class BEVFusionOnnxRuntimeFP16Model(nn.Module):
    def __init__(self, batch_size, device):
        ...
        self.session1 = ort.InferenceSession(f"onnx/img_backbone_fp16.onnx", providers=["CUDAExecutionProvider"])
        self.io1 = self.session1.io_binding()
        self.img_features = torch.empty(batch_size, 6, 256, 32, 88, device=self.device).to(torch.float16)
        self.io1.bind_output(name="img_feats", device_type=self.device, device_id=0, element_type=np.float16, shape=self.img_features.shape, buffer_ptr=self.img_features.data_ptr())

        self.session2 = ort.InferenceSession(f"onnx/vtransform_feature_fp16.onnx", providers=["CUDAExecutionProvider"])
        self.io2 = self.session2.io_binding()
        self.bevpool_features = torch.empty(batch_size, 6, 118, 32, 88, 80, device=self.device).to(torch.float16)
        self.io2.bind_input(name="feats", device_type=self.device, device_id=0, element_type=np.float16, shape=self.img_features.shape, buffer_ptr=self.img_features.data_ptr())
        self.io2.bind_output(name="bevpool_feats", device_type=self.device, device_id=0, element_type=np.float16, shape=self.bevpool_features.shape, buffer_ptr=self.bevpool_features.data_ptr())

        self.bevpool_layer = bev_pool

        self.session3 = ort.InferenceSession(f"onnx/vtransform_downsample_fp16.onnx", providers=["CUDAExecutionProvider"])
        self.io3 = self.session3.io_binding()
        self.img_bev_downsampled = torch.empty(batch_size, 80, 180, 180, device=self.device).to(torch.float16)
        self.io3.bind_output(name="feats_downsampled", device_type=self.device, device_id=0, element_type=np.float16, shape=self.img_bev_downsampled.shape, buffer_ptr=self.img_bev_downsampled.data_ptr())
        ...
        self.session4 = ort.InferenceSession(f"onnx/fusion_fp16.onnx", providers=["CUDAExecutionProvider"])
        self.io4 = self.session4.io_binding()
        self.fused_features = torch.empty(batch_size, 512, 180, 180, device=self.device).to(torch.float16)
        self.io4.bind_input(name="img_bev", device_type=self.device, device_id=0, element_type=np.float16, shape=self.img_bev_downsampled.shape, buffer_ptr=self.img_bev_downsampled.data_ptr())
        self.io4.bind_output(name="fused_bev", device_type=self.device, device_id=0, element_type=np.float16, shape=self.fused_features.shape, buffer_ptr=self.fused_features.data_ptr())
        
        self.session5 = ort.InferenceSession(f"onnx/head_fp16.onnx", providers=["CUDAExecutionProvider"])
        self.io5 = self.session5.io_binding()
        self.reg = torch.empty(batch_size, 2, 200, device=self.device).to(torch.float16)
        self.height = torch.empty(batch_size, 1, 200, device=self.device).to(torch.float16)
        self.dim = torch.empty(batch_size, 3, 200, device=self.device).to(torch.float16)
        self.rot = torch.empty(batch_size, 2, 200, device=self.device).to(torch.float16)
        self.vel = torch.empty(batch_size, 2, 200, device=self.device).to(torch.float16)
        self.heatmap = torch.empty(batch_size, 10, 200, device=self.device).to(torch.float16)
        self.score = torch.empty(batch_size, 10, 200, device=self.device).to(torch.float16)
        self.query_label = torch.empty(batch_size, 200, device=self.device).to(torch.float16)
        self.io5.bind_input(name="feats", device_type=self.device, device_id=0, element_type=np.float16, shape=self.fused_features.shape, buffer_ptr=self.fused_features.data_ptr())
        self.io5.bind_output(name="reg", device_type=self.device, device_id=0, element_type=np.float16, shape=self.reg.shape, buffer_ptr=self.reg.data_ptr())
        self.io5.bind_output(name="height", device_type=self.device, device_id=0, element_type=np.float16, shape=self.height.shape, buffer_ptr=self.height.data_ptr())
        self.io5.bind_output(name="dim", device_type=self.device, device_id=0, element_type=np.float16, shape=self.dim.shape, buffer_ptr=self.dim.data_ptr())
        self.io5.bind_output(name="rot", device_type=self.device, device_id=0, element_type=np.float16, shape=self.rot.shape, buffer_ptr=self.rot.data_ptr())
        self.io5.bind_output(name="vel", device_type=self.device, device_id=0, element_type=np.float16, shape=self.vel.shape, buffer_ptr=self.vel.data_ptr())
        self.io5.bind_output(name="heatmap", device_type=self.device, device_id=0, element_type=np.float16, shape=self.heatmap.shape, buffer_ptr=self.heatmap.data_ptr())
        self.io5.bind_output(name="score", device_type=self.device, device_id=0, element_type=np.float16, shape=self.score.shape, buffer_ptr=self.score.data_ptr())
        self.io5.bind_output(name="query_label", device_type=self.device, device_id=0, element_type=np.float16, shape=self.query_label.shape, buffer_ptr=self.query_label.data_ptr())
    ...
    def forward(self, imgs, points, metas):
        ...
        t0 = time.perf_counter()
        img_fp16 = imgs.to(torch.float16)
        self.io1.bind_input(name="imgs", device_type=self.device, device_id=0, element_type=np.float16, shape=img_fp16.shape, buffer_ptr=img_fp16.data_ptr())
        self.io1.synchronize_inputs()
        self.session1.run_with_iobinding(self.io1)
        elapsed = time.perf_counter() - t0
        ...
        depth = utils.calculate_depth(imgs, points, metas["lidar2img"], metas["img_aug_matrix"], metas["lidar_aug_matrix"])
        depth_fp16 = depth.to(torch.float16)

        t0 = time.perf_counter()
        self.io2.bind_input(name="depth", device_type=self.device, device_id=0, element_type=np.float16, shape=depth_fp16.shape, buffer_ptr=depth_fp16.data_ptr())
        self.io2.synchronize_inputs()
        self.session2.run_with_iobinding(self.io2)
        elapsed = time.perf_counter() - t0
        self._timing['vtransform_feat']['sum']   += elapsed
        ...
        t0 = time.perf_counter()
        img_bev_features = cuda_utils.bev_pool(geom, self.bevpool_features[:len(points)].to(torch.float32), self.bevpool_layer)
        elapsed = time.perf_counter() - t0
        self._timing['vtransform_bevpool']['sum']   += elapsed
        self._timing['vtransform_bevpool']['count'] += 1
        ...
        t0 = time.perf_counter()
        img_bev_features_fp16 = img_bev_features.to(torch.float16)
        self.io3.bind_input(name="feats", device_type=self.device, device_id=0, element_type=np.float16, shape=img_bev_features_fp16.shape, buffer_ptr=img_bev_features_fp16.data_ptr())
        self.io3.synchronize_inputs()
        self.session3.run_with_iobinding(self.io3)
        elapsed = time.perf_counter() - t0
        self._timing['vtransform_down']['sum']   += elapsed
        ...
        t0 = time.perf_counter()
        pts_bev_features_fp16 = pts_bev_features.to(torch.float16)
        self.io4.bind_input(name="pts_bev", device_type=self.device, device_id=0, element_type=np.float16, shape=pts_bev_features_fp16.shape, buffer_ptr=pts_bev_features_fp16.data_ptr())
        self.io4.synchronize_inputs()
        self.session4.run_with_iobinding(self.io4)
        elapsed = time.perf_counter() - t0
        self._timing['fusion']['sum']   += elapsed
...
class BEVFusionTVMModel(nn.Module):
    def __init__(self):
        ...
        self.onnx_model1 = onnx.load("onnx/img_backbone_fp16.onnx")
        self.mod1 = from_onnx(self.onnx_model1, shape_dict={"imgs": [1, 6, 3, 256, 704]}, dtype_dict={"imgs": "float16"})

        self.onnx_model2 = onnx.load("onnx/vtransform_feature_fp16.onnx")
        self.mod2 = from_onnx(self.onnx_model2, shape_dict={"feats": [1, 6, 256, 32, 88], "depth": [1, 6, 1, 256, 704]}, dtype_dict={"feats": "float16", "feats": "float16"})

        self.bevpool_layer = bev_pool

        self.onnx_model3 = onnx.load("onnx/vtransform_downsample_fp16.onnx")
        self.mod3 = from_onnx(self.onnx_model3, shape_dict={"feats": [1, 80, 360, 360]}, dtype_dict={"feats": "float16"},)
        ...
        self.onnx_model4 = onnx.load("onnx/fusion_fp16.onnx")
        self.mod4 = from_onnx(self.onnx_model4, shape_dict={"img_bev": [1, 80, 180, 180], "pts_bev": [1, 256, 180, 180]}, dtype_dict={"img_bev": "float16", "pts_bev": "float16"})
        self.mod4 = Rewriter(CastScalarFloat32To16)(self.mod4)

        self.onnx_model5 = onnx.load("onnx/head_fp16.onnx")
        self.mod5 = from_onnx(self.onnx_model5, shape_dict={"feats": [1, 512, 180, 180]}, dtype_dict={"feats": "float16"})
    ...
    def forward(self, imgs, points, metas):
        ...
        t0 = time.perf_counter()
        bevpool_feats_tvm = self.vm2["main"](img_feats, depth.to(torch.float16))
        ...
        t0 = time.perf_counter()
        img_bev_features = cuda_utils.bev_pool(geom, bevpool_feats[:len(points)].to(torch.float32), self.bevpool_layer)
        elapsed = time.perf_counter() - t0
        self._timing['vtransform_bevpool']['sum']   += elapsed
        self._timing['vtransform_bevpool']['count'] += 1
        ...
        t0 = time.perf_counter()
        fused_bev_tvm = self.vm4["main"](feats_downsampled, pts_bev_features.to(torch.float16))
...
def scatter_nd(var_A: T.handle, B: T.Buffer((T.int64(1), T.int64(10), T.int64(178), T.int64(178), T.int64(4)), "int64"), var_lv11: T.handle, out_buf: T.Buffer((T.int64(1), T.int64(10), T.int64(180), T.int64(180)), "float16")):
    T.func_attr({"op_pattern": 8, "tir.is_scheduled": True, "tir.noalias": True})
    A = T.match_buffer(var_A, (T.int64(1), T.int64(10), T.int64(180), T.int64(180)), dtype="float16", offset_factor=1)
    lv11 = T.match_buffer(var_lv11, (T.int64(1), T.int64(10), T.int64(178), T.int64(178)), dtype="float16", offset_factor=1)
...
@T.prim_func(private=True)
def scatter_nd1(var_lv13: T.handle, B: T.Buffer((T.int64(1), T.int64(1), T.int64(2)), "int64"), var_lv14: T.handle, out_buf: T.Buffer((T.int64(1), T.int64(10), T.int64(180), T.int64(180)), "float16")):
    T.func_attr({"op_pattern": 8, "tir.is_scheduled": True, "tir.noalias": True})
    lv13 = T.match_buffer(var_lv13, (T.int64(1), T.int64(10), T.int64(180), T.int64(180)), dtype="float16", offset_factor=1)
    lv14 = T.match_buffer(var_lv14, (T.int64(1), T.int64(1), T.int64(180), T.int64(180)), dtype="float16", offset_factor=1)
    for i in range(T.int64(324000)):
        with T.block("copy"):
...
@mutator
class CastScalarFloat32To16(PyExprMutator):
    def __init__(self, mod):
        super().__init__(mod)

    def visit_call_(self, call: relax.Call):
        if call.op == tvm.ir.Op.get("relax.nn.batch_norm"):
            new_args = [self.visit_expr(a) for a in call.args]

            bn = relax.Call(call.op, new_args, call.attrs, call.sinfo_args, call.span)

            bld = self.builder_
            y_fp32 = bld.emit(relax.TupleGetItem(bn, 0))
            y_fp16 = bld.emit(relax.op.astype(y_fp32, "float16"))

            mean = bld.emit(relax.TupleGetItem(bn, 1))
            var  = bld.emit(relax.TupleGetItem(bn, 2))

            return relax.Tuple([y_fp16, mean, var])

        return super().visit_call_(call)

            
@tvm.transform.module_pass(opt_level=0, name="Rewriter")
class Rewriter:
    def __init__(self, rewriter_cls):
        self.rewriter_cls = rewriter_cls      

    def transform_module(self, mod, ctx):
        rewriter = self.rewriter_cls(mod)
